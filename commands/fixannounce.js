...
exports.run = async ({message, args}) => {
    if (!args.length) {
        await message.channel.send(embed('RED', "Fix Announce Example", "To use this command, enter the message id followed by the subject and the body. Each argument can be wrapped in quotes if it doesn't fit in a single word.\n\nBelow are some examples on how to fix a announcement.", [
            {
        ...
        if(!args[1] || !args[2]) {return message.channel.send(embed('RED', "Fix Announce Example", "To use this command, enter the message id followed by the subject and the body. Each argument can be wrapped in quotes if it doesn't fit in a single word.\n\nBelow are some examples on how to fix a announcement.", [
            {
				name: 'Example to fix a Announcement',
				value: display({
					name: 'fca',
        ...
        if (!mod) {return message.channel.send(embed("RED", "Fix Announce Error", "You don't have the required role!"))};
