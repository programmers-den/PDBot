...
void rm_role_all_user(struct discord *client, const struct discord_user *bot, const struct discord_message *msg) {
    ...
    if (!guild_member_has_role(guild_member, R_OWNER)) {
        ...
        if ((err_arg = check_if_args_ids(msg, " "))) {
            ...
            snprintf(embed->description, 2049, "Removing %s from %d members", role_mention_str, guild_preview.approximate_member_count);
            ...
            snprintf(embed->description, 2049, "Removed %s from %d members", role_mention_str, guild_preview.approximate_member_count);
