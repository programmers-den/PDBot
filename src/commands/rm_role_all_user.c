...
void rm_role_all_user(struct discord *client, const struct discord_interaction *interaction) {
        ...
        if (!strcmp(interaction->data->options->array[i].name, "id")) {
            ...
            for (size_t j=0; j<guild.members->size; j++) {
                if (!guild.members->array[j].user->bot) {
                    discord_remove_guild_member_role(client, interaction->guild_id, guild.members->array[j].user->id, role_id, &params, NULL);
                }
            }
        }
    }
