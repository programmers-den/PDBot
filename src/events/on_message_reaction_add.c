...
void on_message_reaction_add(struct discord *client, const struct discord_message_reaction_add *event) {
    ...
    get_avatar_url(avatar_url, event->member->user);
    ...
    id_to_str(author_id_str, event->member->user->id);
    user_mention(author_str, event->member->user->id);
    username_and_discriminator_to_str(username_and_discriminator, event->member->user);
    ...
    discord_embed_set_author(&embed, event->member->user->username, NULL, avatar_url, NULL);
    ...
    snprintf(footer_text, sizeof(footer_text), "Author ID: %lu", event->member->user->id);
